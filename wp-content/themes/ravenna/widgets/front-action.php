...
class Ravenna_Action extends WP_Widget {

    function ravenna_action() {
		$widget_ops = array('classname' => 'ravenna_action_widget', 'description' => __( 'A call to action widget', 'ravenna') );
        parent::__construct(false, $name = __('Ravenna: Call to action', 'ravenna'), $widget_ops);
		$this->alt_option_name = 'ravenna_action_widget';
	...
	?>
	<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title', 'ravenna'); ?></label>
	<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></p>
	<p><label for="<?php echo $this->get_field_id('action_text'); ?>"><?php _e('Enter your call to action.', 'ravenna'); ?></label>
	...
	<p><label for="<?php echo $this->get_field_id('action_btn_link'); ?>"><?php _e('Link for the button', 'ravenna'); ?></label>
	...
	<p><label for="<?php echo $this->get_field_id('action_btn_text'); ?>"><?php _e('Title for the button', 'ravenna'); ?></label>
	...
	<label for="<?php echo $this->get_field_id( 'inline' ); ?>"><?php _e( 'Display the button inline with the text?', 'ravenna' ); ?></label></p>
	<?php
	}
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['ravenna_action']) )
			delete_option('ravenna_action');		  
		  
		return $instance;
	}
	
	function flush_widget_cache() {
		wp_cache_delete('ravenna_action', 'widget');
	}
	
	function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'ravenna_action', 'widget' );
		}
...
?>
        <div class="call-to-action <?php echo $aside_style; ?>">
			<?php if ($action_text !='') : ?>
			<h3 class="action-text">
				<?php echo $action_text; ?>
			</h3>
			<?php endif; ?>
			<a href="<?php echo esc_url($action_btn_link); ?>" class="button action-button"><?php echo esc_html($action_btn_text); ?></a>
        </div>
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'ravenna_action', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
